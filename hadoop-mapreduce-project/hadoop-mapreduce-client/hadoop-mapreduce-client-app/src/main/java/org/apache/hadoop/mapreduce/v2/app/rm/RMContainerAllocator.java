...
public class RMContainerAllocator extends RMContainerRequestor
  ...
  private static final HashMap<String, String> containerIdToHostname = new HashMap<String, String>();
...
//      MajorClient.justLog("DEALLOCATE_CONTAINER|" + cId.getApplicationAttemptId().toString() + "|" + aId.toString() + "|" + containerIdToHostname.get(cId.toString()));
  ...
  class ScheduledRequests {
    ...
    private void assignMapsWithLocality(List<Container> allocatedContainers) {
      ...
      MajorClient.justLog("%%% AM 获取taskmap花费 " + (now2 - now1) + " 毫秒");
//      // try to assign to all nodes first to match node local
      ...
      while(it.hasNext() && maps.size() > 0) {
        ...
        String host = allocated.getNodeId().getHost();
        containerIdToHostname.put(allocated.getId().toString(), host);
        if (taskmap.containsKey(allocated.getId().toString())) {
          ...
          if (taskidtotask.containsKey(taskid)) {
            ...
            if (maps.containsKey(t)) {
              ...
              LinkedList<TaskAttemptId> list = mapsHostMapping.get(host);
//              if (list != null && list.size() > 0) {
//                MajorClient.justLog("%%% AM分配: " + allocated.getId().toString() + " 分配Task为 " + taskid + " | 原本计划分配为NodeLocal:" + list.getFirst().toString());
//              } else {
//                String rack = RackResolver.resolve(host).getNetworkLocation();
//                list = mapsRackMapping.get(rack);
//                MajorClient.justLog("%%% AM分配: " + allocated.getId().toString() + " 分配Task为 " + taskid + " | 原本计划分配为RackLocal: " + list.getFirst().toString());
//              }
              if (list != null && list.contains(t)) {
                jce.addCounterUpdate(JobCounter.DATA_LOCAL_MAPS, 1);
                eventHandler.handle(jce);
                hostLocalAssigned++;
                MajorClient.justLog("!!!! hostlocal:" + host + "|" + allocated.getId().toString());
              } else {
                jce.addCounterUpdate(JobCounter.RACK_LOCAL_MAPS, 1);
                eventHandler.handle(jce);
                rackLocalAssigned++;
                MajorClient.justLog("!!!! racklocal:" + RackResolver.resolve(host).getNetworkLocation() + "|" + allocated.getId().toString());
              }
            }
          }
        }
      ...
      MajorClient.justLog("%%% AM 通过taskmap分配task花费 " + (now3 - now2) + " 毫秒");
      // Major end

      // try to assign to all nodes first to match node local
      it = allocatedContainers.iterator();
      if (it.hasNext() && maps.size() > 0) {
        MajorClient.justLog("!!!!AM Container 没分配完！？？1|" + cids.size() + "|" + taskmap.keySet().size());
      }
      boolean nodelocalallocate = false;
      while(it.hasNext() && maps.size() > 0) {
        Container allocated = it.next();
        Priority priority = allocated.getPriority();
        nodelocalallocate = true;
        assert PRIORITY_MAP.equals(priority);
        // "if (maps.containsKey(tId))" below should be almost always true.
        // hence this while loop would almost always have O(1) complexity
        String host = allocated.getNodeId().getHost();
      ...
      if (nodelocalallocate) {
        long now4 = System.currentTimeMillis();
        MajorClient.justLog("%%% AM 默认分配，node local 花费 " + (now4 - now3) + " 毫秒");
      }

      // try to match all rack local
      it = allocatedContainers.iterator();
      if (it.hasNext() && maps.size() > 0) {
        MajorClient.justLog("!!!!AM Container 没分配完！？？2");
      }
      while(it.hasNext() && maps.size() > 0){
        Container allocated = it.next();
        Priority priority = allocated.getPriority();
      ...
      MajorClient.justLog("%%% AM分配完毕，共花费 " + (now5 - now1) + " 毫秒");
