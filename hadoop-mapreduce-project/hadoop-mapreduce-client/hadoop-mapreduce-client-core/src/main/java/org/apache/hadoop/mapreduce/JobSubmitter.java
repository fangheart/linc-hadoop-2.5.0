...
class JobSubmitter {
  ...
  protected static final Log LOG3 = LogFactory.getLog("FangHeart");
  ...
  throws ClassNotFoundException, InterruptedException, IOException {
    ...
    LOG3.info("Fang--1.2获得JobId");
    LOG3.info("Fang--" +"jobStaging Path is " + jobStagingArea);
    LOG3.info("Fang--" +"Ip is " + ip);
    LOG3.info("Fang--" +"jobId is " + jobId);

    job.setJobID(jobId);
    Path submitJobDir = new Path(jobStagingArea, jobId.toString());

    LOG3.info("Fang--" +"submitJobDir is " + submitJobDir);
    ...
    try {
      ...
      LOG3.info("Fang--1.3复制相关配置文件Jar包到stagging");
      ...
      LOG3.info("Fang--1.4将input划分为split，并将split数据和split元数据写入系统工作目录，最后返回split的数");
      int maps = writeSplits(job, submitJobDir);
      conf.setInt(MRJobConfig.NUM_MAPS, maps);
      LOG.info("number of splits:" + maps + "| JobId:" + jobId.getJtIdentifier() + "| NodeId:" + submitHostName);

      LOG3.info("number of splits:" + maps + "| JobId:" + jobId.getJtIdentifier() + "| NodeId:" + submitHostName);

      String id = jobId.toString().substring(4);
      MajorClient.addApplication(maps, id);
      JobConf jConf = (JobConf)job.getConfiguration();
      org.apache.hadoop.mapred.InputFormat fm = jConf.getInputFormat();
      FileInputFormat fm2 = (FileInputFormat)fm;
      fm2.jobid = id;
      org.apache.hadoop.mapred.InputSplit[] splits =
              fm2.getSplits(jConf, jConf.getNumMapTasks());
//      for (org.apache.hadoop.mapred.InputSplit sp : splits) {
//        StringBuilder sb = new StringBuilder();
//        sb.append("Split Location: ");
//        for (String lo : sp.getLocations()) {
//          sb.append(lo);
//          sb.append("|");
//        }
//        sb.append(sp.getLength());
//        try {
//          MajorClient.justLog(sb.toString());
